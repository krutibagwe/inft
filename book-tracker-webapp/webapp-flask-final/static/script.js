...
function addBook() {
    ...
    if (!title || !author || isNaN(totalPages) || isNaN(pagesRead)) {
...
function redirectToFavoriteAuthors() {
...
function addBook() {
    ...
    if (pagesRead > totalPages) {
        alert("Number of pages read cannot be greater than total number of pages in the book.");
        return;
    }
...
function clearProgress() {
    const confirmation = window.confirm("Are you sure you want to clear all progress?");
    if (confirmation) {
        books = [];
        updateBookList();
        saveBooksToLocalStorage();
